...
| **地面** | 救援目标 | `TGT` | 被困人员/待投物资点 | 被动/微弱信标 | 任务相关 |
...
TGT {
  id, type
  position:         (x, y, z)
  target_type:      enum{被困人员, 物资投递点, 设备部署点}
  urgency:          U ∈ [1, 5]     // 紧急度
  time_critical:    T_gold (s)     // 黄金救援窗口
  signal_beacon:    enum{有, 无, 微弱}
  status:           enum{待救援, 救援中, 已完成}
}
```
...
═══ 救援链路 1：指挥车 → 被困点A ═══
  ...
  主路径:    GND-C → BS-1 → UAV-R-1 → UAV-M-1 → TGT-1
  备份路径1: GND-C → UAV-R-2 → UAV-M-2 → UAV-M-1 → TGT-1  (空-空中继)
  备份路径2: GND-C ─── 直达 ──→ UAV-M-1 → TGT-1            (高功率直连)
  热备节点:  UAV-R-1 的备份 = UAV-S-1 (可临时转为中继模式)

═══ 救援链路 2：指挥车 → 被困点B ═══

  主路径:    GND-C → BS-1 → UAV-R-1 → UAV-M-2 → TGT-2
  备份路径1: GND-C → UAV-R-2 → UAV-M-2 → TGT-2
  备份路径2: GND-C → GND-P(中转) → UAV-M-3 → UAV-M-2 → TGT-2
  热备节点:  UAV-R-2 的备份 = UAV-R-1 (交叉备份)

═══ 救援链路 3：指挥车 → 被困点C ═══

  主路径:    GND-C → UAV-R-2 → UAV-M-3 → TGT-3
  备份路径1: GND-C → BS-1 → UAV-R-1 → UAV-M-3 → TGT-3
  备份路径2: GND-C ─── 直达 ──→ UAV-M-3 → TGT-3
...
[T=135s] UAV-R-1 SNR 降至 2dB，判定链路中断
         → 自愈引擎启动
         → 救援链路1 切换至备份路径1 (GND-C → UAV-R-2 → UAV-M-2 → UAV-M-1 → TGT-1)
         → 切换耗时 380ms，救援通信未中断
```
