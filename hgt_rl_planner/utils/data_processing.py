...
def pyg_to_cugraph(pyg_data: Data, directed: bool = True) -> Optional[any]:
    ...
    try:
        ...
        G.from_cudf_edgelist(edge_df, source="source", destination="destination", store_transposed=True)
        return G
    ...
    if torch.cuda.is_available() and HAS_CUGRAPH:
        ...
        if G is not None:
            try:
                ...
                return {int(row['vertex']): float(row['pagerank']) for _, row in pr.to_pandas().iterrows()}
...
def process_custom_kg(
...
) -> Tuple[Data, NodeMap, RelationMap, Dict[int, float], torch.Tensor, Dict[int, Set[int]], Dict[int, str]]:
    """
    处理自定义图，提取异构类型和先修关系。
    ...
    if existing_node_id_map:
        ...
        # 降级：基于名称对齐（风险：重名节点塌缩）
        print("  - 警告：未发现 existing_node_id_map，回退到基于名称对齐 (存在重名塌缩风险)")
        ...
        for node in nodes:
            ...
            if name in name_to_old_ids:
                ...
                potential_ids = [oid for oid in name_to_old_ids[name] if oid not in used_old_ids]
                if potential_ids:
    ...
    for edge in edges:
        src_new, tgt_new = old_id_to_new_id.get(edge['source']), old_id_to_new_id.get(edge['target'])
    ...
    pagerank_values = calculate_pagerank(Data(edge_index=edge_index, num_nodes=num_nodes))
    data = Data(edge_index=edge_index, edge_type=edge_type, num_nodes=num_nodes)
    return data, node_map, relation_map, pagerank_values, node_types, prerequisite_map, int_id_to_raw_id
...
def process_standard_kg(train_triplets, valid_triplets, test_triplets):
    ...
    entities = sorted(list(set(h for h, r, t in all_triplets) | set(t for h, r, t in all_triplets)))
    ...
    def to_int(trips): return [(ent_map[h], rel_map[r], ent_map[t]) for h, r, t in trips]
    t_int, v_int, ts_int = to_int(train_triplets), to_int(valid_triplets), to_int(test_triplets)

    edge_index, edge_type = [], []
    for h, r, t in t_int:
        edge_index.append([h, t]); edge_type.append(r)
        edge_index.append([t, h]); edge_type.append(r + num_rel) # 反向边

    data = Data(edge_index=torch.tensor(edge_index, dtype=torch.long).t().contiguous(),
                edge_type=torch.tensor(edge_type, dtype=torch.long), num_nodes=num_ent)

    return data, ent_map, rel_map, t_int, v_int, ts_int


def convert_to_hetero(data: Data, node_types: torch.Tensor, edge_type: torch.Tensor, relation_map: Dict[str, int]) -> Tuple[Dict[str, torch.Tensor], Dict[tuple, torch.Tensor], tuple]:
    """
...
def clean_graph(data, report=True):
    if data is None or not hasattr(data, 'edge_index'): return data
    ...
    for idx in range(edge_index.shape[1]):
        ...
        seen.add((s, t)); cleaned_pairs.append([s, t])
        if edge_type is not None: cleaned_types.append(edge_type[idx].item())
    new_data = Data(edge_index=torch.tensor(cleaned_pairs, dtype=torch.long).t().contiguous(),
                    edge_type=torch.tensor(cleaned_types, dtype=torch.long), num_nodes=data.num_nodes)
    if report: print(f"--- clean_graph: edges {edge_index.shape[1]} -> {new_data.num_edges}")
    return new_data
...
def filter_reachable_pairs(pairs, data, max_pairs=None, report=True):
    ...
    reachable = []
    for s, t in pairs:
        if s == t: reachable.append((s, t))
        else:
            q, v = deque([s]), {s}
            while q:
                c = q.popleft()
                if any(nb == t for nb in adj.get(c, [])): reachable.append((s, t)); break
                for nb in adj.get(c, []):
                    if nb not in v: v.add(nb); q.append(nb)
        if max_pairs and len(reachable) >= max_pairs: break
    if report: print(f"--- filter_reachable_pairs: {len(reachable)}/{len(pairs)} reachable")
