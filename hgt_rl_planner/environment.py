...
class RLEnvironment:
    ...
    def __init__(self,
                 ...
                 reward_clipping_value: Optional[float] = 0.3,
    ...
    def reset_batch(self, batch_pairs: List[Tuple[int, int]], device) -> List[EpisodeState]:
    ...
    def get_valid_actions(self, state: Optional[EpisodeState] = None) -> List[int]:
        """核心动作空间探测逻辑：支持关系感知约束"""
    ...
    def _compute_reward_internal(self, state: EpisodeState, action: int, current_potential: float, is_prereq_edge: bool = False, is_soft_edge: bool = False) -> Tuple[float, bool, Dict]:
        """
        ...
        if p_skip_count > 0:
            ...
            if is_prereq_edge:
                ...
                reward -= self.REWARD_LAMBDA * self.soft_skip_penalty_scale * capped_skip
            else:
                # 其他边跨级
                reward -= self.REWARD_LAMBDA * self.other_skip_penalty_scale * capped_skip
        ...
        if state.current_node == state.target_node:
            ...
            path_efficiency = (self.max_path_length - state.step_count) / self.max_path_length
    ...
    def step(self, action: int) -> Tuple[int, float, bool, Dict]:
        ...
        if action not in valid_actions:
            ...
            return self.state.current_node, -10.0, True, {
        ...
        reward, done, info = self._compute_reward_internal(self.state, action, p, is_prereq_edge=is_prereq, is_soft_edge=is_soft)
        return self.state.current_node, reward, done, info

    def step_batch(self, 
                   batch_states: List[EpisodeState], 
                   actions: List[Optional[int]], 
                   potentials: np.ndarray) -> List[Tuple[float, bool, Dict]]:
        """批量步进 (带边类型识别)"""
        results = []
        for i, state in enumerate(batch_states):
            ...
            res = self._compute_reward_internal(state, action, float(potentials[i]), is_prereq_edge=is_prereq, is_soft_edge=is_soft)
            results.append(res)
        return results
