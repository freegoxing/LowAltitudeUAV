    ...
    for idx, (start_id, true_target_id) in enumerate(eval_pairs_positive):
        ...
        if not available_negatives:
            ...
            neg_samples = rng.choice(
                available_negatives, num_candidate_neg_samples, replace=False
            )
        else:
            # 如果同类型节点不足，则重复采样
            neg_samples = rng.choice(
                available_negatives, num_candidate_neg_samples, replace=True
            )
        ...
        for cand_id in candidate_targets:
            score, path_found = get_path_score(
                start_id, cand_id, model, env, node_embeddings, device
            )
...
def calculate_mrr_hits(
    ranks: list[int], hits_n: list[int] = [1, 3, 10]
) -> dict[str, float]:
    """
    计算 MRR 和 Hits@N 指标。
    Args:
    ...
    # 计算 Hits@N
    metrics = {
        f"hits@{n}": np.mean([1 if rank <= n else 0 for rank in ranks]) for n in hits_n
    }
    metrics["mrr"] = mrr
    return metrics


def has_path_constrained(
    start_node: int, end_node: int, env: "RLEnvironment", max_depth: int = 15
) -> bool:
    """
...
def get_path_details(
    ...
    # 诊断统计
    path_info = {"total_p_skip": 0, "soft_edge_steps": 0, "prereq_satisfied": True}
...
def get_path_score(
    ...
    """
    _, total_reward, success, _ = get_path_details(
        start_node_id, target_node_id, model, env, node_embeddings, device
    )
...
def evaluate_ranking_metrics(
    ...
    env: "RLEnvironment",
    eval_pairs_positive: list[tuple[int, int]],
    all_known_triplets: set[tuple[int, int, int]]
    | None,  # For filtered ranking in standard datasets
    adj: dict[int, list[int]],  # For has_path filtering
    num_entities: int,
    node_embeddings: torch.Tensor,
    device: torch.device,
    num_candidate_neg_samples: int = 99,  # 1 true + num_candidate_neg_samples false
    evaluation_seed: int | None = None,  # [新增] 评估随机种子，确保负采样确定性
) -> dict[str, float]:
    """
    ...
    for idx, (start_id, true_target_id) in enumerate(eval_pairs_positive):
        ...
        while (
            neg_samples_generated < num_candidate_neg_samples
            ...
            if all_known_triplets:
                ...
                for r_idx in range(
                    len(env.relation_map)
                ):  # 使用 env.relation_map 的长度获取关系总数
        ...
        for cand_target_id in candidate_targets:
            score, path_found = get_path_score(
                start_id, cand_target_id, model, env, node_embeddings, device
            )
        ...
        )  # x[2] (path_found) True>False, x[1] (score) High->Low, -x[0] (target_id) Low->High for deterministic tie-break
...
def compute_path_prereq_metrics(
    ...
    relevant_backbone_path = [
        n for n in backbone_path if n in all_target_ancestors or n == target_id
    ]
    rel_ordered_pairs = 0
    ...
    target_relevant_ppc = (
        rel_consistent_pairs / rel_ordered_pairs if rel_ordered_pairs > 0 else 1.0
    )
    ...
    if not target_frontier_prereqs:
        ...
        covered_count = sum(
            1 for prereq in target_frontier_prereqs if int(prereq) in visited_in_path
        )
...
def evaluate_navigation_metrics(
    ...
    node_embeddings: torch.Tensor,
    device: torch.device,
    metric_prerequisite_map: dict[int, set[int]] | None = None,
) -> dict[str, float]:
    """
    计算增强后的导航指标。
    """
    ...
    for start_node, target_node in eval_pairs:
        ...
        if success:
            ...
            if metric_prerequisite_map:
                metrics = compute_path_prereq_metrics(
                    path, metric_prerequisite_map, target_node
                )
    ...
    avg_path_length = (
        total_path_len / max(success_count, 1) if success_count > 0 else 0.0
    )

    return {
        "success_rate": success_rate,
        "avg_path_length": avg_path_length,
        "path_prereq_compliance": float(np.mean(compliance_rates))
        if compliance_rates
        else 0.0,
        "target_frontier_coverage": float(np.mean(coverage_ratios))
        if coverage_ratios
        else 0.0,
        "target_relevant_ppc": float(np.mean(target_relevant_ppcs))
        if target_relevant_ppcs
        else 0.0,
        "frontier_hit_rate": float(np.mean(frontier_hit_rates))
        if frontier_hit_rates
        else 0.0,
        "conditional_tfc": float(np.mean(conditional_tfcs))
        if conditional_tfcs
        else 0.0,
    }
...
def sample_curriculum_pairs_by_constrained_walk(
    env: "RLEnvironment", num_samples: int = 100, seed: int = 42
) -> list[tuple[int, int]]:
    """
...
def evaluate_prerequisite_compliance(
    paths: list[list[int]], prerequisite_map: dict[int, set[int]]
) -> dict[str, float]:
    """
    评估路径的先修关系合规性。
    Args:
...
def evaluate_path_diversity(paths: list[list[int]]) -> dict[str, float]:
    """
...
def evaluate_educational_metrics(
    ...
    path_quality = analyze_learning_path_quality(
        [path for path in all_paths if len(path) > 1], node_types, type_names
    )
    ...
    if hasattr(env, "prerequisite_map") and env.prerequisite_map:
        compliance = evaluate_prerequisite_compliance(
            [path for path in all_paths if len(path) > 1], env.prerequisite_map
        )
        metrics.update({f"prereq_{k}": v for k, v in compliance.items()})

    # 5. 路径多样性分析
    diversity = evaluate_path_diversity([path for path in all_paths if len(path) > 1])
