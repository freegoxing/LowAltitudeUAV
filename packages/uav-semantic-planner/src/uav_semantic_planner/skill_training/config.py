...
Usage::
    ...
    from uav_semantic_planner.skill_training.config import load_config, flatten_config
...
def is_structured(cfg: dict) -> bool:
    ...
    return any(
        key in _STRUCTURED_SECTIONS and isinstance(cfg.get(key), dict) for key in cfg
    )
...
def flatten_config(cfg: dict) -> dict:
    ...
    if isinstance(env_section, dict):
        mapped_env_keys = {
            k.split(".", 1)[1] for k in _FLATTEN_MAP if k.startswith("env.")
        }
