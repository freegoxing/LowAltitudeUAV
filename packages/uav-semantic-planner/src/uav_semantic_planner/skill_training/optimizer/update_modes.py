...
def payload_label(
    mode: str | None, *, singular: bool = False, title: bool = False
) -> str:
    if is_full_rewrite_minibatch_mode(mode):
...
def short_item_summary(
    item: dict, mode: str | None, *, max_chars: int | None = None
) -> dict[str, Any]:
    if is_full_rewrite_minibatch_mode(mode):
        return {
            "title": str(item.get("title", "")),
            "change_summary": [str(x) for x in item.get("change_summary", [])]
