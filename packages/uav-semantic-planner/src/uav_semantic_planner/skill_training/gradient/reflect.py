...
from uav_semantic_planner.skill_training.model import chat_optimizer
from uav_semantic_planner.skill_training.optimizer.meta_skill import (
...
from uav_semantic_planner.skill_training.prompts import load_prompt
from uav_semantic_planner.skill_training.utils import extract_json
    ...
    for idx, item in enumerate(items, 1):
        ...
        if not user_prompt:
            user_prompt_path = os.path.join(
                prediction_dir, tid, "target_user_prompt.txt"
            )
        ...
        if os.environ.get("REFLACT_CODEX_TRACE_TO_OPTIMIZER", "0") == "1":
            ...
            if not codex_trace_summary:
                codex_trace_summary_path = os.path.join(
                    prediction_dir, tid, "codex_trace_summary.txt"
                )
...
def _resolve_prompt(
    custom: str | None, default_name: str, update_mode: str = "patch"
) -> str:
...
def run_minibatch_reflect(
    ...
    *,
    ...
    failures = [
        r for r in results if not r.get("hard") or float(r.get("hard", 0)) < 1e-9
    ]
    ...
    with ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for i, fut in enumerate(as_completed(futs), 1):
            ...
            n_edits = len(
                get_payload_items(patch.get("patch", {}) if patch else {}, update_mode)
            )
            print(
