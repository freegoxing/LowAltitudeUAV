...
from uav_semantic_planner.skill_training.datasets.base import BatchSpec
from uav_semantic_planner.skill_training.envs.base import EnvAdapter
from uav_semantic_planner.skill_training.evaluation.gate import (
    GateResult,
    evaluate_gate,
    select_gate_score,
)
from uav_semantic_planner.skill_training.gradient.aggregate import merge_patches
from uav_semantic_planner.skill_training.model import (
    chat_optimizer,
...
from uav_semantic_planner.skill_training.optimizer.appendix import (
...
from uav_semantic_planner.skill_training.optimizer.clip import rank_and_select
from uav_semantic_planner.skill_training.optimizer.lr_autonomous import (
    decide_autonomous_learning_rate,
)
from uav_semantic_planner.skill_training.optimizer.meta_skill import run_meta_skill
from uav_semantic_planner.skill_training.optimizer.rewrite import (
    rewrite_skill_from_suggestions,
)
from uav_semantic_planner.skill_training.optimizer.scheduler import build_scheduler
from uav_semantic_planner.skill_training.optimizer.skill import apply_patch_with_report
from uav_semantic_planner.skill_training.optimizer.skill_aware import (
...
from uav_semantic_planner.skill_training.optimizer.slow_update import (
...
from uav_semantic_planner.skill_training.optimizer.update_modes import (
    get_payload_items,
...
from uav_semantic_planner.skill_training.utils import compute_score, skill_hash
...
def _flush_skill_aware_appendix(
    ...
    if consolidate_threshold > 0 and len(after_notes) > consolidate_threshold:
        ...
        if compacted and len(compacted) < len(after_notes):
            current_skill = append_to_appendix_field(
                _strip_all_appendix_fields(current_skill),
                compacted,
            )
...
def _normalise_lr_control_mode(mode: str | None) -> str:
    ...
    if raw not in aliases:
        raise ValueError(
            "optimizer.lr_control_mode must be one of fixed, autonomous, none"
        )
    return aliases[raw]
...
def _build_longitudinal_pairs(
    *,
    ...
    candidates = [
        item for item in candidates if str(item.get("id", "")) not in seen_ids
    ]
...
def _resolve_train_size(cfg: dict, dataloader) -> int:
    ...
    if dataloader is not None:
        ...
        if callable(getter):
            ...
            try:
                inferred = len(dataloader.train_items)
            except Exception:
                inferred = None
...
def _compute_task_type_buckets(
    results: list[dict], task_types: list[str]
) -> dict[str, dict]:
    ...
    def train(self) -> dict:
        ...
        def _build_eval_env(split: str, env_num: int, seed: int):
            if dataloader is None:
                ...
                actual_n = (
                    len(env_manager) if hasattr(env_manager, "__len__") else env_num
                )
        ...
        configure_azure_openai(
            endpoint=(
                cfg.get("azure_openai_endpoint") or cfg.get("azure_endpoint") or None
            ),
            api_version=(
            ...
            api_key=(
                cfg.get("azure_openai_api_key") or cfg.get("azure_api_key") or None
            ),
            auth_mode=cfg.get("azure_openai_auth_mode") or None,
            ad_scope=cfg.get("azure_openai_ad_scope") or None,
            managed_identity_client_id=cfg.get(
        ...
        configure_claude_code_exec(
            ...
            effort=cfg.get(
                "claude_code_exec_effort", cfg.get("reasoning_effort", "medium")
            ),
        ...
        os.environ["REFLACT_CODEX_TRACE_TO_OPTIMIZER"] = (
            "1"
            if target_backend == "codex_exec"
            and cfg.get("codex_trace_to_optimizer", False)
            else "0"
        )
        ...
        lr_control_mode = _normalise_lr_control_mode(
            cfg.get("lr_control_mode", "fixed")
        )
        ...
        rewrite_max_completion_tokens = int(
            cfg.get("rewrite_max_completion_tokens", 64000)
        )
        if batch_size <= 0:
        ...
        print(
            f"  [config] batches/epoch={batches_per_epoch} "
        ...
        if runtime_state:
            ...
            current_skill_path = runtime_state.get(
                "current_skill_path"
            ) or os.path.join(
                out_root,
                "skills",
            ...
            best_score = float(
                runtime_state.get("best_score", current_score) or current_score
            )
        ...
        print(
            ...
            + (f" mixed_weight={gate_mixed_weight}" if gate_metric == "mixed" else "")
            + (
                ""
                if use_gate
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for step_in_epoch in range(steps_per_epoch):
                ...
                for a in range(accumulation):
                    ...
                    t_phase = time.time()
                    print(
                        f"    [1/6 ROLLOUT] train items={train_n} (from pool, batch_seed={batch_seed})"
                    )
                ...
                agg_hard = sum(
                    b["hard"] * b["n_envs"] for b in accum_rollout_stats
                ) / max(total_n, 1)
                agg_soft = sum(
                    b["soft"] * b["n_envs"] for b in accum_rollout_stats
                ) / max(total_n, 1)
                ...
                print(
                    f"    [3/6 done] merged {n_edits_merged} {payload_label(update_mode)}"
                )
                ...
                if update_mode == "rewrite_from_suggestions":
                    ...
                    if rewrite_result and rewrite_result.get("new_skill"):
                        ...
                        apply_report = []
                        with open(
                            os.path.join(step_dir, "rewrite_result.json"), "w"
                        ) as f:
                    ...
                    selected_candidate = next(
                        (
                            ...
                            if isinstance(item, dict)
                            and str(item.get("new_skill", "")).strip()
                        ),
                        None,
                    )
                    if selected_candidate:
                        candidate_skill = (
                            str(selected_candidate["new_skill"]).rstrip() + "\n"
                        )
                        apply_report = []
                        rewrite_result = {
                            "reasoning": ranked_patch.get("reasoning", ""),
                            "change_summary": selected_candidate.get(
                                "change_summary", []
                            ),
                        ...
                        with open(
                            os.path.join(step_dir, "full_rewrite_result.json"), "w"
                        ) as f:
                            json.dump(
                                {
                ...
                if apply_report:
                    with open(
                        os.path.join(step_dir, "edit_apply_report.json"), "w"
                    ) as f:
                ...
                if rewrite_result:
                    step_rec["rewrite_change_summary"] = rewrite_result.get(
                        "change_summary", []
                    )
                if apply_report:
                ...
                if (
                    update_mode == "rewrite_from_suggestions" and rewrite_result is None
                ) or (
                ...
                if gate.action == "accept_new_best":
                    ...
                    print(
                        f"    [6/6 EVALUATE] FORCE-ACCEPT (gate disabled) {score_label}"
                    )
                else:
                    print(
            ...
            if use_slow:
                ...
                if os.path.exists(slow_done_path):
                    ...
                    slow_items = (
                        list(slow_env) if hasattr(slow_env, "__iter__") else slow_env
                    )
                    print(
                        f"    [slow update] sampled {len(slow_items)} train items (seed={slow_seed})"
                    )
                    ...
                    results_prev = adapter.rollout(
                        slow_env, prev_skill, prev_rollout_dir
                    )
                    results_curr = adapter.rollout(
                        slow_env, current_skill, curr_rollout_dir
                    )
                    ...
                    n_regressed = sum(
                        1 for p in comparison_pairs if p["category"] == "regressed"
                    )
                    n_improved = sum(
                        1 for p in comparison_pairs if p["category"] == "improved"
                    )
                    n_persist = sum(
                        1
                    ...
                    n_stable = sum(
                        1 for p in comparison_pairs if p["category"] == "stable_success"
                    )
                    print(
                    ...
                    if slow_result and slow_result.get("slow_update_content"):
                        ...
                        with open(
                            os.path.join(slow_dir, "candidate_skill.md"), "w"
                        ) as f:
            ...
            if use_meta_skill:
                meta_skill_dir = os.path.join(
                    out_root, "meta_skill", f"epoch_{epoch:02d}"
                )
                meta_skill_done_path = os.path.join(
                    meta_skill_dir, "meta_skill_result.json"
                )
                ...
                if os.path.exists(meta_skill_done_path):
                    ...
                    if epoch_comparison_pairs is None:
                        ...
                        meta_items = (
                            list(meta_env)
                            if hasattr(meta_env, "__iter__")
                            else meta_env
                        )
                        prev_rollout_dir = os.path.join(meta_skill_dir, "rollout_prev")
                        curr_rollout_dir = os.path.join(meta_skill_dir, "rollout_curr")
                        results_prev = adapter.rollout(
                            meta_env, prev_skill, prev_rollout_dir
                        )
                        results_curr = adapter.rollout(
                            meta_env, epoch_last_step_skill, curr_rollout_dir
                        )
                        epoch_comparison_pairs, all_meta_comparison_pairs = (
                            _build_longitudinal_pairs(
                                adapter=adapter,
                                dataloader=dataloader,
                                prev_skill=prev_skill,
                        ...
                        if all_meta_comparison_pairs is not epoch_comparison_pairs:
                            save_comparison_pairs(
                                ...
                                os.path.join(
                                    meta_skill_dir, "comparison_pairs_all.json"
                                ),
                            )
                        save_comparison_pairs(
                    ...
                    if meta_skill_result and meta_skill_result.get(
                        "meta_skill_content"
                    ):
                        ...
                        print(
                            f"    [meta skill] no memory produced, {meta_skill_time}s"
                        )
        ...
        if cfg["eval_test"]:
            ...
            try:
                if skill_hash(current_skill) == skill_hash(best_skill):
                    ...
                    final_selection_hard, final_selection_soft = compute_score(
                        fval_results
                    )
                    final_gate_score = select_gate_score(
                        final_selection_hard,
                        final_selection_soft,
                        gate_metric,
                        gate_mixed_weight,
                    )
                    print(
            ...
            baseline_test_results = adapter.rollout(
                test_env, skill_init, baseline_test_dir
            )
            baseline_test_hard, baseline_test_soft = compute_score(
                baseline_test_results
            )
            baseline_buckets = _compute_task_type_buckets(
                baseline_test_results, task_types
            )
            ...
            try:
                if skill_hash(current_skill) == skill_hash(best_skill):
                    ...
                    print(f"\n{'=' * 60}")
                    print(
                        "  FINAL SKILL TEST — evaluate last skill on Test set (valid_unseen)"
                    )
                    print(f"{'=' * 60}")
                    ...
                    final_test_dir = os.path.join(out_root, "test_eval_final")
                    os.makedirs(final_test_dir, exist_ok=True)
                    final_test_results = adapter.rollout(
                        test_env3, current_skill, final_test_dir
                    )
                    final_test_hard, final_test_soft = compute_score(final_test_results)
                    final_buckets = _compute_task_type_buckets(
                        final_test_results, task_types
                    )
                ...
                epoch_stats.append(
                    {
                        ...
                        "accepts": sum(
                            1 for h in epoch_records if "accept" in h.get("action", "")
                        ),
                        "rejects": sum(
                            1 for h in epoch_records if h.get("action") == "reject"
                        ),
                        "skips": sum(
                            1
                            for h in epoch_records
                            if h.get("action") == "skip_no_patches"
                        ),
                        "best_score_at_epoch_end": epoch_records[-1].get(
                            "best_score", 0.0
                        ),
                        "current_score_at_epoch_end": epoch_records[-1].get(
                            "current_score", 0.0
                        ),
                    }
                )
        ...
        print(f"{'=' * 60}")
        print(
            f"  steps={len(history)} accept={n_accept} reject={n_reject} skip={n_skip}"
        )
        print(
            f"  best_score={best_score:.4f} (step {best_step})  wall={total_wall:.0f}s"
        )
        if epoch_stats:
