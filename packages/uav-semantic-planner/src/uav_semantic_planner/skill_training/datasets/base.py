...
class BaseDataLoader(ABC):
    ...
    @staticmethod
    def make_base_seeds(
        steps_per_epoch: int, accumulation: int, seed: int
    ) -> list[int]:
...
def _parse_split_ratio(text: str) -> tuple[int, int, int]:
    ...
    if len(parts) != 3:
        raise ValueError(f"split_ratio must be in train:val:test form, got {text!r}")
    try:
        train, val, test = (int(part) for part in parts)
    except ValueError as exc:
        raise ValueError(f"split_ratio must contain integers, got {text!r}") from exc
...
def _compute_split_counts(
    total: int, ratio: tuple[int, int, int]
) -> tuple[int, int, int]:
    weights = list(ratio)
    denom = sum(weights)
...
class SplitDataLoader(BaseDataLoader):
    ...
    def _resolve_split_output_dir(self, cfg: dict) -> str:
        ...
        env_name = str(
            cfg.get("env") or type(self).__name__.replace("DataLoader", "").lower()
        )
        ratio_tag = str(self.split_ratio or "2:1:7").replace(":", "-")
        return os.path.join(
            out_root,
            "_generated_splits",
            f"{env_name}_{ratio_tag}_seed{self.split_seed}",
        )
    ...
    def load_raw_items(self, data_path: str) -> list[dict]:
        ...
        if os.path.isdir(data_path):
            if any(
                os.path.isdir(os.path.join(data_path, name)) for name in SPLIT_NAMES
            ):
                raise ValueError(
    ...
    def _materialize_ratio_split(self, cfg: dict) -> str:
        ...
        with open(
            os.path.join(split_dir, "split_manifest.json"), "w", encoding="utf-8"
        ) as f:
