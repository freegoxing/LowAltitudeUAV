...
from uav_semantic_planner.skill_training.model import chat_target
...
def _rollout_one(
    item: dict[str, Any], skill_content: str, max_completion_tokens: int
) -> dict[str, Any]:
...
def run_batch(
    *,
    ...
    if worker_count == 1:
        results = [
            _rollout_one(item, skill_content, max_completion_tokens) for item in items
        ]
    else:
            ...
            results = list(
                executor.map(
                    lambda item: _rollout_one(
                        item, skill_content, max_completion_tokens
                    ),
                    items,
                )
            )
