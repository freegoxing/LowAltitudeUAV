...
from uav_semantic_planner.skill_training.model.backend_config import (
...
def render_skill_md(
    ...
    *,
    ...
    body = (
        skill_content.strip()
        or "No additional dynamic guidance was provided for this task."
    )
    chunks = [
        "---",
        f'name: "{name}"',
...
def prepare_workspace(
    *,
    ...
    os.makedirs(
        os.path.join(work_dir, ".agents", "skills", "skillopt-target"), exist_ok=True
    )

    skill_path = os.path.join(
        work_dir, ".agents", "skills", "skillopt-target", "SKILL.md"
    )
...
def _normalize_tools(allowed_tools: list[str] | str | None) -> str:
    ...
    if isinstance(allowed_tools, str):
        return ",".join(
            part.strip() for part in allowed_tools.split(",") if part.strip()
        )
...
def _validate_exec_path(path: str) -> str:
    ...
    if denied:
        raise ValueError(
            f"Refusing to expose denied data directory to exec backend: {', '.join(sorted(denied))}"
        )
    return resolved


def _validated_add_dirs(
    work_dir: str, data_dirs: list[str] | None, images: list[str] | None
) -> list[str]:
...
def _run_claude_code_sdk_exec(
    *,
    ...
    async def _query() -> tuple[str, str]:
        ...
        if model:
            options.model = (
                model.split("/", 1)[1] if model.startswith("anthropic/") else model
            )
        ...
        terminal_is_error = (
            bool(getattr(last, "is_error", False)) if last is not None else False
        )
        raw = _json_dumps(
            {
                "backend": "claude_code_sdk",
                "uuid": first_data.get("uuid", "")
                if isinstance(first_data, dict)
                else "",
                "session_id": getattr(last, "session_id", "")
                if last is not None
                else "",
                "model": first_data.get("model", model)
                if isinstance(first_data, dict)
                else model,
                ...
                "total_cost_usd": getattr(last, "total_cost_usd", 0.0)
                if last is not None
                else 0.0,
                ...
                or (terminal_is_error and not response.strip()),
...
def _run_claude_code_cli_exec(
    *,
    ...
    if images:
        for image in images:
            cmd.extend(
                ["--add-dir", _validate_exec_path(os.path.dirname(image) or work_dir)]
            )
    ...
    *,
    ...
    async def _query() -> tuple[str, str]:
        ...
        turn = await thread.run(
            prompt, {"output_schema": _strict_schema(ANSWER_SCHEMA)}
        )
...
def _run_codex_cli_exec(
    *,
    ...
    actual_full_auto = (
        bool(config.get("full_auto", True)) if full_auto is None else bool(full_auto)
    )
    ...
    try:
        ...
        raise
    try:
        from uav_semantic_planner.skill_training.model import azure_openai as _openai
