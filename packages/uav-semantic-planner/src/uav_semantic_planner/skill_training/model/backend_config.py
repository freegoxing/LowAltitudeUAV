...
OPTIMIZER_BACKEND = normalize_backend_name(
    os.environ.get("OPTIMIZER_BACKEND", "openai_chat")
)
...
CODEX_EXEC_NETWORK_ACCESS = _parse_bool(
    os.environ.get("CODEX_EXEC_NETWORK_ACCESS"), False
)
...
EXEC_EMPTY_RESPONSE_RETRIES = max(
    0, _parse_int(os.environ.get("EXEC_EMPTY_RESPONSE_RETRIES"), 1)
)
...
def configure_codex_exec(
    *,
    ...
    if network_access is not None:
        ...
        os.environ["CODEX_EXEC_NETWORK_ACCESS"] = (
            "true" if CODEX_EXEC_NETWORK_ACCESS else "false"
        )
    if web_search is not None:
        CODEX_EXEC_WEB_SEARCH = bool(web_search)
        os.environ["CODEX_EXEC_WEB_SEARCH"] = (
            "true" if CODEX_EXEC_WEB_SEARCH else "false"
        )
...
def configure_claude_code_exec(
    *,
    ...
    if max_thinking_tokens is not None:
        ...
        os.environ["CLAUDE_CODE_EXEC_MAX_THINKING_TOKENS"] = str(
            CLAUDE_CODE_EXEC_MAX_THINKING_TOKENS
        )
