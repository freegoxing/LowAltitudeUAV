    ...
    def stage_snapshot(self, stage: str) -> dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "total_tokens": d.get("prompt_tokens", 0)
    ...
    for attempt in range(retries):
        try:
            if _needs_responses_api(deployment):
                ...
                if tools:
                    kwargs["tools"] = [
                        _chat_tool_to_responses_tool(tool) for tool in tools
                    ]
...
def configure_azure_openai(
    *,
    ...
    resolved_optimizer_auth_mode = (
        _clean(optimizer_auth_mode, lower=True) or shared_auth_mode
    )
    ...
    resolved_target_mi = (
        _clean(target_managed_identity_client_id) or shared_managed_identity_client_id
    )
