...
from uav_semantic_planner.skill_training.model.common import (
...
CLAUDE_ALLOW_ATTACHMENT_READ = os.environ.get(
    "CLAUDE_ALLOW_ATTACHMENT_READ", "1"
...
def _content_to_text(
    content: Any, attachments: list[dict[str, Any]], *, image_counter: int
) -> tuple[str, int]:
    ...
    *,
    ...
    for message in messages:
        ...
        text, image_counter = _content_to_text(
            message.get("content", ""), attachments, image_counter=image_counter
        )
        if role == "system":
            if text.strip():
        ...
        if role == "assistant":
            ...
            if tool_calls:
                ...
                block += "\n  Compatibility tool requests:\n" + json.dumps(
                    simplified_calls, ensure_ascii=False, indent=2
                )
        ...
        if role == "tool":
            ...
            history_parts.append(
                _history_line(f"Tool result (tool_call_id={tool_call_id})", text)
            )
            continue
    ...
    if tools:
        ...
        prompt_parts.append(
            "Do not execute these compatibility tools yourself. If you need one, request it in `tool_calls`. Each `arguments` field must be a JSON string."
        )
        if tool_choice == "required":
            ...
            prompt_parts.append(
                f"Tool choice policy: you must request the compatibility tool `{function.get('name', '')}`."
            )
    ...
    if structured_output:
        ...
        if tools:
            prompt_parts.append(
                "Set `content` to the assistant-visible reply. Set `tool_calls` to an empty array when no compatibility tool is needed."
            )
    else:
...
def _copy_attachments_to_temp(
    attachments: list[dict[str, Any]], temp_dir: str
) -> list[dict[str, str]]:
    ...
    for index, attachment in enumerate(attachments, 1):
        ...
        if source_path:
            ...
            target_path = os.path.join(
                temp_dir, f"image_{index}{source_suffix or '.bin'}"
            )
            shutil.copyfile(source_path, target_path)
            copied.append(
                {"path": target_path, "label": str(attachment.get("label", ""))}
            )
            continue
...
def _append_attachment_instructions(
    prompt: str, copied_attachments: list[dict[str, str]]
) -> str:
    ...
    lines = [
        ...
        *[
            f"- {item['label'] or f'Attached image {index}'}: {item['path']}"
            for index, item in enumerate(copied_attachments, 1)
        ],
...
def _check_claude_error(stderr_text: str, model: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(
            "Claude CLI is not logged in. Run `claude auth login` (or start `claude` and use `/login`) first."
        )
    if (
        ...
        raise RuntimeError(
            f"Claude backend tried to use model {model!r}, but your current Claude CLI/account rejected it. Try an available Claude model such as {default_model!r}."
        )
...
def _call_messages(
    ...
    *,
    ...
    system, prompt, attachments = _build_prompt_from_messages(
        messages, tools=tools, tool_choice=tool_choice, structured_output=return_message
    )
    model = deployment or TARGET_DEPLOYMENT
    last_err = None
    for attempt in range(retries):
        try:
            raw_text, payload, usage_info = _run_claude_print(
                system=system,
                prompt=prompt,
                model=model,
                tools=tools,
            ...
            if return_message:
                return _compat_message_from_payload(
