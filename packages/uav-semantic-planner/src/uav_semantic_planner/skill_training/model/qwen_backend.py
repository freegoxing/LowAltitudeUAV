...
def _initial_config(role: str) -> QwenChatConfig:
    ...
    deployment_env = (
        "OPTIMIZER_DEPLOYMENT" if role == "optimizer" else "TARGET_DEPLOYMENT"
    )
...
def _compat_message_from_payload(
    message: dict[str, Any], choice: dict[str, Any]
) -> CompatAssistantMessage:
...
def _post_chat_completion(
    ...
    try:
        with urllib.request.urlopen(
            req, timeout=timeout or config.timeout_seconds
        ) as resp:
    ...
    try:
        ...
        raise RuntimeError(
            f"Qwen chat API returned non-JSON response: {raw[:1000]}"
        ) from e
