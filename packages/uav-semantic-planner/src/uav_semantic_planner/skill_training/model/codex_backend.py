...
def _compat_message_from_payload(
    ...
    *,
    ...
    if tool_choice == "required" and not tool_calls:
        raise RuntimeError(
            "Codex response did not request a tool under tool_choice='required'"
        )
    ...
    for attempt in range(retries):
        try:
            ...
            raw_text, usage_info = _run_codex_exec(
                ...
                output_schema=_assistant_message_schema()
                if structured_output
                else None,
                timeout=timeout,
            )
            ...
            last_err = (
                RuntimeError(f"Codex CLI timed out after {timeout}s")
                if timeout
                else exc
            )
