...
def refine_levels(
    ...
    """
    print(f"=== 启动 LLM 等级细化与幻觉修正 (Graph-based Refinement) - 领域: {target_field} ===")
    
    # 动态构建输入输出路径
    levels_file = os.path.join(data_dir, f"concept_levels_{target_field}.json")
    if not output_file:
        output_file = os.path.join(data_dir, f"concept_levels_{target_field}_refined.json")
    ...
    if not os.path.exists(levels_file):
        print(f"错误: 缺少定级文件 {levels_file}，请确保已运行 rank_concepts_by_level.py --field '{target_field}'")
        return
    ...
    if unmatched_in_graph:
        print(f"警告: 图中有 {len(unmatched_in_graph)} 个节点在定级文件中未找到，将使用默认等级 5")
    if unmatched_in_levels:
        print(f"注意: 定级文件中有 {len(unmatched_in_levels)} 个 ID 未在当前图中出现，将被忽略")
    ...
    for node_id in common_ids:
        ...
        if llm_lvl >= 6 and score > 0.3:
            ...
            adjustments.append((id_to_name[node_id], llm_lvl, suggested_lvl, score, "LLM偏难(图显示为基础)"))
            
        # Case B: LLM 认为太易，但图结构显示它很高级
        elif llm_lvl <= 3 and score < -0.5:
            suggested_lvl = min(8, llm_lvl + 3)
            adjustments.append((id_to_name[node_id], llm_lvl, suggested_lvl, score, "LLM偏易(图显示为进阶)"))
    ...
    if adjustments:
        print("Top 5 修正案例:")
        # 按修正幅度排序
        sorted_adjusts = sorted(adjustments, key=lambda x: abs(x[1]-x[2]), reverse=True)
        for name, old, new, score, reason in sorted_adjusts[:5]:
            print(f"  - {name}: L{old} -> L{new} (TopoScore: {score:.2f}) | 原因: {reason}")
...
def main():
    ...
    parser.add_argument("--data_dir", type=str, default="data/MOOCCubex", help="数据目录")
