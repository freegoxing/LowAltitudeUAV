...
def draw_task_communication_subgraph(
    ...
    for idx, flow_result in enumerate(planning_result["flow_results"]):
        ...
        primary_edges = list(zip(primary_path[:-1], primary_path[1:], strict=False))
        if primary_edges:
            nx.draw_networkx_edges(
                nx_graph,
                pos,
        ...
        for backup_idx, backup_path in enumerate(backup_paths):
            backup_edges = list(zip(backup_path[:-1], backup_path[1:], strict=False))
            if not backup_edges:
                continue
            nx.draw_networkx_edges(
                nx_graph,
        ...
        for backup_path in backup_paths:
            for u, v in zip(backup_path[:-1], backup_path[1:], strict=False):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Visualize UAV task communication subgraph"
    )
    ...
    parser.add_argument(
        "--output_img",
        type=str,
        default="visualizations/task_communication_subgraph.png",
    )
    parser.add_argument(
        "--output_json",
        type=str,
        default="apps/web/src/data/generated-task-communication-subgraph.json",
    )
    ...
    with open(output_json, "w", encoding="utf-8") as output_file:
        json.dump(planning_result, output_file, ensure_ascii=False, indent=2)
    print(f"✅ 任务通信子图决策已导出: {output_json}")
