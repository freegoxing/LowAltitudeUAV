...
from uav_semantic_planner.agents.agent1.assessor import SituationAssessor
from uav_semantic_planner.agents.agent2.translator import MockAgent2_DecisionTranslator
...
def main():
    ...
    print(
        "  >>> [SkillOpt] 正在根据大量模拟数据优化 Agent 1 的技能文档 (agent1_skill.md)..."
    )
    time.sleep(1)
    ...
    print("\n  >>> [HGT] 构建异构图网络缓存特征...")
    os.system(
        f"uv run python {os.path.join(project_root, 'scripts/build_uav_graph.py')}"
    )
    ...
    os.system(
        f"uv run python {os.path.join(project_root, 'scripts/train_uav_policy.py')} --epochs 50 --w_snr 0.5 --w_btnk 5.0 --w_stab 0.0"
    )
    ...
    print("\n[阶段 2: 态势判别 (Agent 1)]")
    print(
        "  🚨 突发事件：侦察节点 [UAV-S-1] 通过热成像发现【人员被困】！触发态势评估机制。"
    )

    skill_path = os.path.join(
        project_root,
        "packages/uav-semantic-planner/src/uav_semantic_planner/agents/agent1/skills/initial.md",
    )
    agent1 = SituationAssessor(skill_path=skill_path)

    # 从图谱提取当前的统计特征 (示例数据)
    mock_graph_stats = {"avg_snr": 8.5, "disconn_count": 2}
    # 传感器回传的环境信息
    mock_env_info = {"battery_level": 0.60, "wind_condition": "strong"}
    # 目标特征信息
    mock_target_info = {
    ...
    model_pt_path = os.path.join(
        project_root, "checkpoints/UAV_Demo/uav_policy_final.pt"
    )
    graph_pt_path = os.path.join(
        project_root, "checkpoints/UAV_Demo/uav_hetero_graph.pt"
    )

    planner = UAVRoutingPlanner(
        model_pt_path=model_pt_path, graph_pt_path=graph_pt_path, device="cpu"
    )

    mission_spec = MockAgent2_DecisionTranslator.translate_to_mission_spec(
        situation_level, task, kg_data["mission_candidate_groups"]
    )
