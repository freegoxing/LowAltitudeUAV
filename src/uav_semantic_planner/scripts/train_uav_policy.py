...
def main():
    ...
    data = Data(edge_index=full_edge_index.to(device), edge_type=full_edge_type.to(device), num_nodes=total_nodes)
    ...
    print(f"--- 任务模式权重: SNR增益={args.w_snr}, 瓶颈容忍={args.w_btnk}, 稳定性={args.w_stab} ---")
    ...
    # 4. 准备训练集 (采样可达的端到端通信对)
    from uav_semantic_planner.utils.evaluation_lib import sample_uav_communication_pairs, evaluate_navigation_metrics
    ...
    if not target_pairs:
        raise ValueError("未能采样到任何可达通信对，请检查图是否连通或 SNR 阈值是否过高")
    ...
    for b in range(1, num_batches + 1):
        ...
        if b % 10 == 0:
            logging.info(f"Batch {b}/{num_batches} | Reward: {avg_reward:.2f} | SR: {sr:.2%} | Loss: {float(loss):.4f}")
