...
class MockAgent2_DecisionTranslator:
    ...
    @staticmethod
    def translate_to_reward_weights(situation_level: str, task_type: str) -> list[float]:
        """
        ...
        if task_type == "高清视频图传":
            ...
            print("  > 分析推理: 飞控指令属于时延与可靠性敏感型，要求木桶短板极高且绝对稳定。")
            w_snr = 0.2
            w_btnk = 4.0
            w_stab = 3.0
            ...
            if "Level 1" in situation_level:
                print("  > 分析推理: 当前环境极度恶劣，搜救广播只求能连通即可，重罚断连。")
                w_snr = 0.5
                w_btnk = 5.0
                w_stab = 0.0
            else:
        ...
        print(f"  > 下发底层 RL 奖励权重: W=[SNR增益:{w_snr}, 瓶颈容忍:{w_btnk}, 稳定性:{w_stab}]")
...
def simulate_workflow():
    ...
    for task in tasks:
        ...
        if task == "高清视频图传":
            print("  ✓ RL 探索收敛倾向: 优先挑选距离基站近、无遮挡的主干链路，哪怕这些链路偶尔不稳定。")
        else:
            print("  ✓ RL 探索收敛倾向: 放弃峰值高的短直飞链路，选择通过多个 UAV-R (中继) 绕开干扰源，寻找最平稳的安全链路。")
        print("-" * 40)
